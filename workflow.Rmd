...
Now we want to join these distances to our other data. We'll start by getting the 2017 5-year ACS population estimates for the counties using the [tidycensus](https://walkerke.github.io/tidycensus/) package, and then we'll merge that with the TBI data. This will be saved as `mergedToTBI.csv`.
...
```{r readMerged, echo=F}
dist_TBI <- read_csv("Data/mergedToTBI.csv")
glimpse(dist_TBI)
```
...
p1 <- dist_TBI %>%
  ...
  ggplot(aes(x=pop17, fill=Missing)) +
...
```{r lm}
fit <- lm(Crude ~ dist_cent+log(pop17), data=dist_TBI) 
summary(fit)
...
fit_impute <- dist_TBI_imputed %>%
  lm(Crude ~ dist_cent + log(pop17), data=.)
summary(fit_impute)
```
...
```{r}
sjPlot::plot_models(fit, fit_impute, fit_worst, std.est = "std2", rm.terms="log(pop17)",
...
library(BAS)
set.seed(1234)
modBayes <- bas.lm(Crude ~ dist_cent+pop17,
                     data = dist_TBI,
                     method = "MCMC",
                     prior = "ZS-null",
                     modelprior = uniform())
