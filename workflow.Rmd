...
Now we can geocode these locations
...
if(F){
  ...
  ## First pass
  trauma_R1 <- traumaCenters %>%
    mutate_geocode(Address, output="more")
  
  ## Handle the errors
  trauma_R2 <- trauma_R1 %>%
    ## limit to failed geocodes or appx address
    filter(is.na(loctype)|loctype=="approximate") %>%
    
    ## search for center name + city + st + zip
    mutate(query = str_extract(Address, "[^,]+, [:upper:]{2} \\d{5}")) %>%
    mutate(query = str_glue("{CenterName}, {query}")) %>%
    select(query, Address:LevelFull) %>%
    
    ## geocode it
    mutate_geocode(query, output="more")
  
  ## Merge
  traumaGeocoded <- trauma_R1 %>%
    ## exclude the ones that had to be re-coded
    filter(!is.na(loctype) & loctype!="approximate") %>%
    bind_rows(select(trauma_R2, -query)) %>%
    select(-north, -south, -east, -west)
  
  ## Write to csv  
  traumaGeocoded %>% write_csv("Data/TraumaCenters_geocoded.csv")
}
...
centers_geo <- read_csv("Data/TraumaCenters_geocoded.csv")
```
...
# Sensitivity analysis using the NPI associated address
provs_sens <- read_csv("Google_results/geo_npi_byAddress.csv") %>%
  filter(geoState!="hi", geoState!="ak") %>%
  st_as_sf(coords = c("lon", "lat"), crs=4269)
```
...
## Sensitivity analysis using NPI addresses
dist_cent_sens <- find_closest(US_counties, provs_sens, centroid = T) %>%
  st_drop_geometry() %>%
  select(GEOID, dist_cent_sens=Distance) 
dist_min_sens  <- find_closest(US_counties, provs_sens, centroid = F) %>%
  st_drop_geometry() %>%
  select(GEOID, dist_min_sens=Distance) 
...
dist %>% 
  inner_join(dist_min_sens) %>% 
  inner_join(dist_cent_sens) %>%
  write_csv("Data/Distances_by-names-sens.csv")

rm(dist_cent, dist_min, dist_cent_sens, dist_min_sens)
```

```{r readDistances, echo=F}
dist <- read_csv("Data/Distances_by-names.csv")
glimpse(dist)
dist_sens <- read_csv("Data/Distances_by-names-sens.csv")
```
...
##
## Update: 2020-03-16 (v3)
##         Adding sensitivity analysis columns


mergedToTBI <- dist_sens %>%
...
mergedToTBI %>%
  left_join(nProvs) %>%
  write_csv("Data/mergedToTBI_v3.csv") # Write to csv
