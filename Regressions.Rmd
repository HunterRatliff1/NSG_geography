...
Reading in `Data/mergedToTBI_v2.csv`, and dropping any varaibles with NA's. I'm also linking in the census regions and divisions (more on this below) to attempt to account for some of the geographical variance^[I don't have much experience with modeling geographic data, so perhaps advice from Dr Prochaska would be warranted] (see the last section of this report).
...
df_full_sens <- read_csv("Data/mergedToTBI_v3.csv") %>%
  select(-npi, -providerCity, -providerST) %>%
  select(GEOID:Crude, dist_cent_sens, dist_min_sens) %>%
  filter(!is.na(Deaths)) %>% # drop counties with missing rates
  filter(GEOID != "46102") %>% # GEOID didn't match to ACS data
  left_join(ST_regions) %>%
  mutate(region   = factor(region),
