...
df_full <- read_csv("https://raw.githubusercontent.com/HunterRatliff1/NSG_geography/master/public_data/regression.csv") %>%
  ...
  mutate(male   = male * 100,
         Over65 = Over65 *100) %>%
  # mutate(Urban = case_when(between(RUCC_2013, 1, 3)  ~ "Urban",
  #                          RUCC_2013 %in% c(4, 6, 8) ~ "Adjacent",
  #                          RUCC_2013 %in% c(5, 7, 9) ~ "Rural",
  #                          TRUE                      ~ "error")) %>%
...
Univariate descriptive statistics for each variable can be found in **Table 1**. The mean (95% CI) mortality rate among all counties was `r frmtci(mean_ci(df$Crude))` per 100,000 population (**Figure 1**). The mean (95% CI) distance to the nearest neurosurgeon was `r frmtci(mean_ci(df$dist_cent))` miles (**Figure 2**). 
...
Counties with missing rates also tended to have smaller populations (mean $\pm$ SD: `r print_mean_sd(subset(df_compare_missing, Exclude)$StdPop)`) than counties without missing data (`r print_mean_sd(subset(df_compare_missing, !Exclude)$StdPop)`; p<0.001), and all together counties with missing rates only accounted for 1.67% of the US population. 
...
Crude mortality rates were the lowest in metropolitan counties (`r nrow(subset(df, Urbanization=="Metro"))` counties; mean +/- SD rate: `r print_mean_sd(subset(df, Urbanization=="Metro")$Crude)`), followed by non-metropolitan counties (`r nrow(subset(df, Urbanization=="Nonmetro"))` counties; `r print_mean_sd(subset(df, Urbanization=="Nonmetro")$Crude)`), with rural counties having the highest rate (`r nrow(subset(df, Urbanization=="Rural"))` counties; `r print_mean_sd(subset(df, Urbanization=="Rural")$Crude)`); p < 0.001. 
...
tibble(dist_cent = c(0,0),
       male      = c(.0, .0),
       Over65    = c(.0,.01),
       Urbanization = c("Metro", "Metro")
) %>%
  predict(base_fit, newdata = .)

3.183063 - 2.531114

df %>% mutate(dist_cent = dist_cent/10) %>%
  lm(Crude~dist_cent, data=.) %>%
...
```{r}
# fit
fit_slr   <- lm(Crude  ~ dist_cent, data=df) 

base_fit %>% summary()

# Age-adjusted
update(base_fit, AgeAdj~.) %>% summary()

# Using min distance instead of center
update(base_fit, .~. + dist_min - dist_cent) %>% summary()
update(base_fit, .~. + dist_min - dist_cent, subset = dist_min!=0) %>% summary()

fit       <- lm(Crude  ~ dist_cent + log(pop12) + male + Over65 + Urbanization, data=df)
fit_min   <- lm(Crude  ~ dist_min + log(pop12) + male + Over65 + Urbanization, data=df)
fit_age   <- lm(AgeAdj ~ dist_cent + log(pop12) + male + Over65 + Urbanization, data=df)
fit_sens  <- lm(Crude  ~ dist_cent_sens + log(pop12) + male + Over65 + Urbanization, data=df)

fit_subset <- lm(Crude  ~ dist_min + log(pop12) + male + Over65 + Urbanization, data=df,
                 subset = dist_min!=0) 
summary(fit)
summary(fit_subset)
